class TemplateSystem:
    ...
    def render_page(self, title, modules_html, system_info=None, alerts=None):
        """Render complete responsive page with modules"""
        if alerts is None:
            alerts = []
            
        # Create Javascript alert injection
        js_alerts = ""
        if alerts:
            js_list = [ '"{}"'.format(a) for a in alerts ]
            # Using simple browser alert() for maximum compatibility
            js_alerts = """
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    let messages = [""" + ",".join(js_list) + """];
                    alert(messages.join('\\n\\n'));
                });
            </script>
            """
