...
from adafruit_httpserver import Response, ChunkedResponse
import re
...
class SDManagerModule(WicdpicoModule):
    def __init__(self, foundation):
        ...
        try:
            ...
            if self.card_available:
                ...
                self.foundation.add_alert("Warning: SD Card Not Detected. Settings override disabled.")
        except Exception as e:
            self.card_available = False
            self.foundation.startup_print("SD card initialization failed: " + str(e))
            self.foundation.add_alert("Error: SD Card Initialization Failed. Check Hardware.")
    ...
    def open_file(self, request):
        print("DEBUG: Request to open file received.")
        try:
            ...
            try:
                ...
                filesize = stats[6]
                print("DEBUG: File size: {} bytes".format(filesize))
            except Exception as e:
                print("DEBUG: Stat failed: {}".format(e))
                return Response(request, "Error: File not found.", content_type="text/plain")

            if filesize > 5000:
                print("DEBUG: Streaming tail via ChunkedResponse...")
                def tail_generator():
                    yield "File: {}\n\n".format(filename).encode('utf-8')
                    yield "--- LARGE FILE DETECTED ({} bytes) ---\n".format(filesize).encode('utf-8')
                    yield "--- Showing last 2KB only to protect RAM ---\n".encode('utf-8')
                    yield "--- Download file to SD card to verify full data ---\n\n".encode('utf-8')
                    
                    with open(filename, "rb") as f:
                        f.seek(filesize - 2000)
                        # Yield in even smaller 512b chunks for absolute safety
                        while True:
                            chunk = f.read(512)
                            if not chunk: break
                            yield chunk
                ...
                return ChunkedResponse(request, tail_generator, content_type="text/plain")

            print("DEBUG: Streaming small file via ChunkedResponse...")
            ...
            return ChunkedResponse(request, full_file_generator, content_type="text/plain")

        except Exception as e:
            print("DEBUG: CRITICAL ERROR: {}".format(e))
    ...
    def download_file(self, request):
        try:
            ...
            # Generates chunks of data from the file
            def file_chunk_generator():
                with open(filepath, "rb") as f:
                    # Small 512b chunks are safest for memory on the Pico
                    chunk = bytearray(512)
                    while True:
                        read_count = f.readinto(chunk)
                        if read_count == 0:
                            break
                        # Explicitly yield bytes to comply with ChunkedResponse requirements
                        yield bytes(chunk[:read_count])
                        gc.collect()

            headers = {
                "Content-Disposition": 'attachment; filename="{}"'.format(filename)
            }
            # Verified CircuitPython pattern for large file streaming
            return ChunkedResponse(request, file_chunk_generator, content_type="text/plain", headers=headers)
    ...
    def get_html_template(self):
        ...
        <script>
            ...
            function openSelectedFile() {{
                ...
                console.log('Sending request for:', selectedFile);
                ...
                .then(text => {{
                    ...
                    const content = separatorIndex !== -1 ? text.substring(separatorIndex + 2) : text;
                ...
                .catch(err => alert('Browser Error: ' + err.message));
            }}
